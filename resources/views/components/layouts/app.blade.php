...
<head>
    ...
    <title>@yield('title', 'Koperasi MikroLink')</title>
    
    {{-- CSS & FONTS --}}
    <script src="https://cdn.tailwindcss.com"></script>
    <link href="https://fonts.googleapis.com/css2?family=Plus+Jakarta+Sans:wght@400;500;600;700;800&display=swap" rel="stylesheet">
    
    {{-- CRITICAL: Load Styles Paling Atas --}}
    @livewireStyles
    @fluxStyles
...
<body>
    ...
    {{-- Tampilkan Sidebar Hanya Jika Login --}}
    @auth
    ...
    <main class="{{ auth()->check() ? 'main-content' : '' }}">
        <flux:main>
            {{-- Slot untuk Livewire Volt (Penting untuk Fitur Manajemen Simpanan) --}}
            @isset($slot)
                {{ $slot }}
            @endisset

            {{-- Yield untuk Blade biasa --}}
            @yield('content')
        </flux:main>
    </main>
    ...
    {{-- CRITICAL: Load Scripts Paling Bawah & Jangan Double Alpine --}}
    @livewireScripts
    @fluxScripts
