...
new #[Layout('layouts.app')] class extends Component {
    ...
    public string $filterStatus = 'Baru';
    ...
    public function lihatDetail(int $id): void
    {
        ...
        $this->adminNote = $this->selectedLoan->admin_note ?? '';
    }
    ...
    public function approve(int $id): void
    {
        ...
        $loan->update([
            ...
            'admin_note'         => $this->adminNote ?: 'Pengajuan telah diverifikasi oleh Admin.',
            'admin_reviewed_at'  => now(),
            'progress_percentage'=> 50,
        ]);
        $this->tutupModal();
        $this->dispatch('notif', type: 'success', message: 'Pengajuan disetujui, menunggu review Manajer!');
    }
    ...
    public function reject(int $id): void
    {
        $this->validate([
            ...
            'adminNote.required' => 'Catatan wajib diisi saat menolak.',
        ...
        $this->tutupModal();
        $this->dispatch('notif', type: 'error', message: 'Pengajuan pinjaman ditolak.');
    }
};
?>
        ...
        <div class="bg-white rounded-[32px] border border-gray-100 shadow-sm overflow-hidden mb-6">
            <div class="p-8 bg-gray-50/30 border-b border-gray-50">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h2 class="text-xl font-bold text-gray-900">Review Pengajuan Pinjaman — Admin</h2>
                        <p class="text-sm text-gray-500 mt-0.5">Tahap 1 dari 2: Verifikasi awal pengajuan pinjaman anggota.</p>
                    </div>
                </div>
            </div>
            ...
            <div class="px-8 py-4 flex gap-2">
                @foreach (['Baru' => 'Baru', 'Dalam Review' => 'Dalam Review', 'Disetujui' => 'Disetujui', 'Ditolak' => 'Ditolak', 'ALL' => 'Semua'] as $value => $label)
                    <button
                        type="button"
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse ($loans as $loan)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <span class="px-2.5 py-1 rounded-md text-[10px] font-extrabold bg-blue-50 text-blue-600 uppercase tracking-widest">
                                        {{ $loan->type }}
                                    </span>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-xs font-bold text-gray-600">{{ $loan->progress_percentage }}%</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <button
            ...
            <div class="relative bg-white rounded-[28px] shadow-2xl w-full max-w-md overflow-hidden z-10"
                ...
                <div class="px-5 py-4 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        ...
                        <h3 class="font-bold text-gray-900 text-sm">Detail Pengajuan</h3>
                    </div>
                    ...
                    <div class="flex items-center justify-between p-3 bg-gray-50 rounded-2xl">
                        ...
                        <span class="px-2.5 py-1 rounded-lg text-[10px] font-extrabold uppercase tracking-widest
                            {{ $selectedLoan->status === 'Disetujui' ? 'bg-emerald-50 text-emerald-600' :
                               ($selectedLoan->status === 'Ditolak' ? 'bg-red-50 text-red-600' :
                               ($selectedLoan->status === 'Dalam Review' ? 'bg-amber-50 text-amber-600' : 'bg-gray-50 text-gray-600')) }}">
                            {{ $selectedLoan->status }}
                        </span>
                    </div>
                    ...
                    @if ($selectedLoan->status === 'Baru')
                        <div>
                            ...
                            <textarea
                                ...
                                class="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 transition-all resize-none"
                            ></textarea>
                            @error('adminNote')
                    ...
                    @else
                        @if ($selectedLoan->admin_note)
                            <div class="p-3 bg-gray-50 rounded-xl">
                                ...
                                <p class="text-sm text-gray-700">{{ $selectedLoan->admin_note }}</p>
                            </div>
                        @endif
                    @endif
                </div>
