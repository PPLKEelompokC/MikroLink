    ...
    @if ($selectedDeposit)
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative bg-white rounded-[28px] shadow-2xl w-full max-w-md overflow-hidden z-10"
                ...
                @else
                    {{-- ✅ Footer untuk status APPROVED / REJECTED --}}
                    <div class="px-5 py-4 border-t border-gray-100 flex gap-2">
                        @if ($selectedDeposit->status === 'APPROVED')
                            <a href="{{ route('simpanan.akta.download', $selectedDeposit->id) }}"
                                target="_blank"
                                class="flex-1 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 rounded-xl transition-colors text-sm flex items-center justify-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <button
                            type="button"
                            wire:click="tutupModal"
                            class="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-2.5 rounded-xl transition-colors text-sm">
                            Tutup
                        </button>
                    </div>
                @endif
